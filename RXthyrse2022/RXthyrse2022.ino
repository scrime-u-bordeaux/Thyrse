...
void loop() {
  ...
  if (radio.available()) {             // is there a payload? get the pipe number that recieved it
    ...
    int joyX = (128 * buffer[1] + buffer[2]) - 8192;
    usbMIDI.sendPitchBend(joyX, 1);
    int joyY = (128 * buffer[3] + buffer[4]) - 8192;
    usbMIDI.sendPitchBend(joyY, 2);
    int accX = (128 * buffer[9] + buffer[10]) - 8192;
    usbMIDI.sendPitchBend(accX, 3);
    int accY = (128 * buffer[7] + buffer[8]) - 8192;
    usbMIDI.sendPitchBend(accY, 4);
    int accZ = (128 * buffer[5] + buffer[6]) - 8192;
...
lastRX = millis();
    ...
    for (uint8_t j = 0; j < SIZE; ++j) {
      Serial.print(buffer[j], HEX); Serial.print("_");
    }
    Serial.println();
#endif
  }
